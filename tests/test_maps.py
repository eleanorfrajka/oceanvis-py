...
PYGMT_SKIP_REASON = (
    "PyGMT or GMT not available" if not HAVE_PYGMT else "PyGMT is available"
)
...
def create_test_bathymetry_file(temp_dir):
    ...
    ds = xr.Dataset(
        {"waterdepth": (("lat", "lon"), waterdepth)}, coords={"lat": lat, "lon": lon}
    )

    # Add attributes
    ds.waterdepth.attrs = {"units": "meters", "long_name": "Water depth"}
...
class TestBathymetryDataHandling:
    ...
    def test_different_depth_variable_names(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            ds = xr.Dataset(
                {"depth": (("lat", "lon"), depth)}, coords={"lat": lat, "lon": lon}
            )
